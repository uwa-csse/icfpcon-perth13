...
let rec evalExpr (x,y,z) body = 
        ...
        | If0(cond, ifExpr, elseExpr) -> 
            if (evalExpr env cond = 0UL) then evalExpr env ifExpr else evalExpr env elseExpr
        | Fold((init, acc), lambda) -> 
            let foldee = System.BitConverter.GetBytes(evalExpr env init)
            let accBytes = evalExpr env acc
            Array.fold (fun s t -> evalExpr (x, uint64 t, s) lambda) accBytes foldee
        | Unary(op, arg) -> 
